...
public static class PhysicsTools
{
    ...
    public static int SphereCastDownwards(Vector3 origin, float radius, float distance, int layerMask, int ringAmount, int ringResolution, RaycastHitPlus[] hitBuffer, bool debug = false)
    {
        int hitAmount = 0;
        float ringSizes = radius / ringAmount;
        float ringOffsetAngle = (360f / ringResolution) / ringAmount; //offset every ring a little to cover more space
        ...
        if (debug)
        {
            Debug.DrawLine(firstPoint, firstPoint + Vector3.up * -distance);
        }
        ...
        for (int r = 0; r < ringAmount; r++) //ring
        {
            ...
            for (int l = 0; l < ringResolution; l++) //lap of ring or ring resolution
            {
                ...
                Vector3 point = Quaternion.AngleAxis(lr * 360f + ringOffsetAngle * r, Vector3.up) * ringVector;
                point += origin;

                if (debug)
                {
                    Debug.DrawLine(point, point + Vector3.up * -distance);
                }
