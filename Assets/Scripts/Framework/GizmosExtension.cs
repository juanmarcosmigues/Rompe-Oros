...
namespace Utils
{
	public static class GizmosExtensions
	{
		...
		public static void DrawCross(Vector3 position, float length = 0.25f)
		{		
			Gizmos.DrawLine(position - Vector3.right * length, position + Vector3.right * length);
			Gizmos.DrawLine(position - Vector3.forward * length, position + Vector3.forward * length);
		}
