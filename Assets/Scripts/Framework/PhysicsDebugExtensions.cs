//Script by:
//HiddenMonk: https://discussions.unity.com/t/how-can-you-visualize-a-boxcast-boxcheck-etc/161101/2
...
public static class PhysicsDebugExtensions
{
    ...
    // Draws the full sphere cast from origin to distance
        ...
        int segments = 16)
    {
        ...
        DrawSphere(end, radius, color, segments);

        // Draw guide lines along cast direction
        Debug.DrawLine(start + Vector3.up * radius, end + Vector3.up * radius, color);
        Debug.DrawLine(start - Vector3.up * radius, end - Vector3.up * radius, color);
        ...
        int segments)
    {
        ...
        for (int i = 0; i <= segments; i++)
        {
            ...
            Vector3 point =
                center +
                (Mathf.Cos(angle) * axisA + Mathf.Sin(angle) * axisB) * radius;
